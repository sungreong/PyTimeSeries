...
class args:
    ...
    n_epoch = 10000
    n_log_interval = 100
    save_folder_name = "m_transformer_1_step"
...
def plot_and_loss(eval_model, data_source, epoch, folder, device):
    eval_model.eval()
    total_loss = 0.0
    ...
    with torch.no_grad():
        for i in range(0, len(data_source) - 1):
            ...
            output = eval_model(data.to(device))
...
def predict_future(eval_model, data_source, steps, folder):
    eval_model.eval()
    total_loss = 0.0
    test_result = torch.Tensor(0)
    truth = torch.Tensor(0)
    ...
    with torch.no_grad():
        for i in range(0, steps):
            output = eval_model(data[-args.input_window :].to(args.device)).cpu()
    ...
    plt.plot(data[: args.input_window], color="blue")
    plt.grid(True, which="both")
    plt.axhline(y=0, color="k")
    fig_path = f"{folder}/future{steps:05d}.png"
    filepath = CurDir.joinpath(fig_path)
    filepath.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(filepath)
    plt.close()
...
min_, max_ = df["Adj_Close"].min(), df["Adj_Close"].max()
min_value = 0.9 * min_
max_value = 1.1 * min_
...
train_data["transed_Adj_Close"] = scaler.transform(train_data[["Adj_Close"]].values)
valid_data["transed_Adj_Close"] = scaler.transform(valid_data[["Adj_Close"]].values)


model = TransAm(feature_size=100, num_layers=args.num_layers, dropout=args.dropout, nhead=args.nhead).to(args.device)
...
best_model = None
train_inputs = create_inout_sequences(train_data["transed_Adj_Close"].values, tw=args.input_window)
valid_inputs = create_inout_sequences(valid_data["transed_Adj_Close"].values, tw=args.input_window)
...
for epoch in pbar:
    ...
    if epoch % args.n_log_interval == 0:
        va_loss = plot_and_loss(model, valid_inputs, epoch, args.save_folder_name, args.device)
        predict_future(model, valid_inputs, 100, args.save_folder_name)
